/**
 * Script to import archived tickets from CSV for Jovan Gajic
 * Run with: node scripts/importArchivedTicketsJovan.js
 */
...
// Config
const CSV_PATH = path.join(__dirname, '../../total_scores_jovangajic.csv');
const JOVAN_USER_ID = '68eba4db74a236db0ffe13b4';
...
const getOrCreateAgent = async (agentName) => {
  ...
  if (agent) {
    ...
    agent = await Agent.create({
      name: agentName,
      createdBy: new mongoose.Types.ObjectId(JOVAN_USER_ID),
...
const importTickets = async () => {
  console.log('Starting import for Jovan Gajic...\n');
